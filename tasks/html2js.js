...
module.exports = function(grunt) {
  ...
  var compileTemplate = function(moduleName, filepath, quoteChar, indentString, useStrict) {
    ...
    var module = '.run([' + quoteChar + '$templateCache' + quoteChar + ', function($templateCache) ' +
    ...
    this.files.forEach(function(f) {
      ...
      var modules = f.src.filter(existsFilter).map(function(filepath) {
        ...
        if (options.target === 'js') {
          result = compileTemplate(moduleName, filepath, options.quoteChar, options.indentString, options.useStrict);
        } else if (options.target === 'coffee') {
          result = compileCoffeeTemplate(moduleName, filepath, options.quoteChar, options.indentString);
        } else {
        ...
        bufName = name;
        var head = 'angular.module(' + options.quoteChar + name + 'Tpl' + options.quoteChar + ', [])';
        var foot = ';\n'
        grunt.file.write(f.dest + name + '.tpl.js' , grunt.util.normalizelf(head + bufResult + foot));
      }).join('\n');
